...
let 缓存userID = null, 缓存userID已初始化 = false;
...
export default {
	async fetch(request, env, ctx) {
		...
		if (!缓存userID已初始化) {
			const _userIDMD5 = await MD5MD5(管理员密码 + 加密秘钥);
			const _envUUID = env.UUID || env.uuid;
			缓存userID = (_envUUID && uuidRegex.test(_envUUID)) ? _envUUID.toLowerCase() : [_userIDMD5.slice(0, 8), _userIDMD5.slice(8, 12), '4' + _userIDMD5.slice(13, 16), '8' + _userIDMD5.slice(17, 20), _userIDMD5.slice(20)].join('-');
			缓存userID已初始化 = true;
		}
		...
		if (缓存SOCKS5白名单 === null) {
			...
			白名单正则缓存 = 缓存SOCKS5白名单.map(p => new RegExp("^" + p.replace(/\*/g, ".*") + "$", "i"));
			白名单正则已初始化 = true;
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			} else if (!(env.UUID || env.uuid)) return fetch(Pages静态页面 + '/noKV').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }) });
		}
...
async function 读取XHTTP首包(reader, token) {
	...
	while (true) {
		...
		// ponytail: VLESS (byte compare) before Trojan (sha224)
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, request = null) {
	...
	if (启用SOCKS5反代 && (启用SOCKS5全局反代 || 白名单正则已初始化 && 白名单正则缓存.some(r => r.test(host)))) {
...
async function MD5MD5(文本) {
	if (MD5MD5缓存.has(文本)) return MD5MD5缓存.get(文本);

	const 编码器 = new TextEncoder();

	const 第一次哈希 = await crypto.subtle.digest('MD5', 编码器.encode(文本));
	const 第一次十六进制 = binToHex(new Uint8Array(第一次哈希));

	const 第二次哈希 = await crypto.subtle.digest('MD5', 编码器.encode(第一次十六进制.slice(7, 27)));
	const 第二次十六进制 = binToHex(new Uint8Array(第二次哈希));

	const 结果 = 第二次十六进制.toLowerCase();
	if (MD5MD5缓存.size < 2048) MD5MD5缓存.set(文本, 结果);
	return 结果;
}
...
async function DoH查询(域名, 记录类型, DoH解析服务 = "https://cloudflare-dns.com/dns-query") {
	const DoH缓存键 = 域名 + ":" + 记录类型 + ":" + DoH解析服务;
	const 缓存条目 = DoH缓存.get(DoH缓存键);
	if (缓存条目 && Date.now() - 缓存条目.t < 120000) return 缓存条目.d;
	...
	try {
		...
		const dcKey = 域名 + ':' + 记录类型 + ':' + DoH解析服务;
		if (answers.length && DoH缓存.size < 512) DoH缓存.set(dcKey, { d: answers, t: Date.now() });
		return answers;
	} catch (error) {
		const 耗时 = (performance.now() - 开始时间).toFixed(2);
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	const 当前时间 = Date.now();
	if (缓存配置JSON && !重置配置 && 当前时间 - 缓存配置时间 < 配置缓存TTL && 缓存配置JSON.HOST === hostname) {
		缓存配置JSON.TIME = new Date().toISOString();
		缓存配置JSON.gRPCUserAgent = UA;
		缓存配置JSON.优选订阅生成.TOKEN = await MD5MD5(hostname + userID);
		return 缓存配置JSON;
	}
	const _p = 特征码字典[0];
	...
	缓存配置JSON = config_JSON; 缓存配置时间 = Date.now();
	return config_JSON;
}
...
function sha224(s) {
	...
	if (sha224缓存.size < 1024) sha224缓存.set(s, hex);
	return hex;
}
